...
install:
	@vsix=$$(ls -t epic-lens-*.vsix 2>/dev/null | grep -v openvsx | head -1); \
...
publish-marketplace:
	@vsix=$$(ls -t epic-lens-*[0-9].vsix 2>/dev/null | grep -v openvsx | head -1); \
...
# Publish to Open VSX
publish-openvsx:
	@vsix=$$(ls -t epic-lens-*-openvsx.vsix 2>/dev/null | head -1); \
...
clean:
	rm -rf dist/ webview-ui/dist/ epic-lens-*.vsix
